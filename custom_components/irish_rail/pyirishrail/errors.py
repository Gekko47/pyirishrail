...
The hierarchy follows the principle in Skill 02: never return an empty
list merely because the server failed. Every API failure path raises one
of these subclasses, so the integration layer can map it to a typed
``UpdateFailed`` and surface a clear "sensor unavailable" state in the
Home Assistant UI.
"""
