...
* **Bundled seed** (``stops_matrix.json``, generated by
...
def reset_bundled_seed_cache() -> None:
    ...
    Called after the runtime rebuild button has rewritten
    ``stops_matrix.json`` so freshly merged route knowledge reaches the
    config flow immediately instead of after the next process restart.
    """
    global _SEED_CACHE
    _SEED_CACHE = None
